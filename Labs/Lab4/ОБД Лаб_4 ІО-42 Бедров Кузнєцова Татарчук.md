...
```
Оскільки інформація про керівника відділу міститься у таблиці wing, маємо об'єднати worker та wing щоб вивести імена керівників.
...
```
Аналогічно до 10. Тепер  поєднання відбувається із використанням right join.
...
```
Використовуючи HAVING MAX(salary) отримаємо позиції з найбільшою платнею, потім групуємо за position_in_wing.
...
```
Поєднуємо таблиці review та user_ (full outer join) та рахуємо кількість відгуків для кожного користувача.
...
```
WHERE дозволяє порівняти рейтинг кожного релізу із попередньо визначеним середнім рейтингом.
