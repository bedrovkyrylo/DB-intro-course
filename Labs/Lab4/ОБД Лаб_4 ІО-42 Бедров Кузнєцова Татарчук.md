...
Київ — 2026
***
## Завдання з агрегаційними функціями

```
-- 1. середня платня у відділі
SELECT position_in_wing, AVG(salary)
FROM job
GROUP BY position_in_wing;
```
Ми визначаємо середнє значення для усіх значень salary що мають однаковий  position_in_wing використовуючи агрегатну функцію AVG.

![1.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/1.png)
```
-- 2. мінімальна зарплата у відділі
SELECT position_in_wing, MIN(salary)
FROM job
GROUP BY position_in_wing;
```
Аналогічно до завдання 1. Використовуємо агрегатну функцію MIN.

![2.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/2.png)
```
-- 3. сумарна зарплата на відділ
SELECT position_in_wing, SUM(salary)
FROM job
GROUP BY position_in_wing;
```
Аналогічно до завдання 1. Використовуємо агрегатну функцію SUM.

![3.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/3.png)
```
-- 4. мінімальний рейтинг проєкту
SELECT project_release, MIN(rating)
FROM release_
GROUP BY project_release;

```
Альтернативний приклад із таблицею release_ і використанням MIN.

![4.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/4.png)
```
-- 5. середній рейтинг для всіх версій проєкту
SELECT project_release, AVG(rating)
FROM release_
GROUP BY project_release;
```
Альтернативний приклад із таблицею release_ і використанням AVG.

![5.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/5.png)
```
-- 6. середня робоча сила
SELECT AVG(workforce)
FROM wing;
```
Середнє значення з усієї колонки workforce у таблиці wing.

![6.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/6.png)
```
-- 7. кількість користувачів кожного проєкту
SELECT tested_project, COUNT(tester)
FROM outside_testers
GROUP BY tested_project;
```
Кількість тестувальників кожного проєкту (версії об'єднані GROUP BY).

![7.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/7.png)
## Завдання з різними типами джоінів

```
-- 8. кількість працівників, що посідають однакову посаду
SELECT j.job_name, COUNT(c.worker)
FROM contract c
	inner join job j ON c.job=j.job_id
GROUP BY j.job_name;
```
Спершу  поєднуємо таблиці contract та job за колонкою job_id, потім рахуємо COUNT(c.worker). 

![8.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/8.png)
```
-- 9. ВИВЕСТИ ТАБЛИЦЮ contract з іменами працівників, назвами посад та їхнім рангом
...
```
Поєднуємо три таблиці (contract, job та worker) із використанням left join.

![9.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/9.png)
```
-- 10. вивести працівників і продемонструвати, які працівники є керівниками і яких відділів
SELECT wo.worker_name, wi.wing_name
FROM worker wo
	full outer join wing wi ON wo.worker_id=wi.wing_head
```
Оскільки інформація про керівника відділу міститься у таблиці wing, маємо об'єднати worker та wing (full outer join) щоб вивести імена керівників.

![10.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/10.png)
```
-- 11. вивести контактну інформацію всіх керівників відділів
SELECT wo.worker_name, wo.email, wi.wing_name
FROM worker wo
	right join wing wi ON wi.wing_head=wo.worker_id;
```
Тепер  поєднання відбувається із використанням right join. Як наслідок виводимо тільки працівників, що є керівниками.

![11.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/11.png)
## Запити з використанням підзапитів

```
-- 12. знайти відділи, у яких максимальна патня > 4000
SELECT position_in_wing, MAX(salary) AS max_salary
FROM job
GROUP BY position_in_wing
HAVING MAX(salary)>4000;
```
Групуємо за position_in_wing, потім використовуючи HAVING MAX(salary) отримаємо позиції з найбільшою платнею.

![12.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/12.png)
```
-- 13. вивести користувачів, що лишили >2 відгуків
...
```
Поєднуємо таблиці review та user_ (full outer join) та рахуємо кількість відгуків для кожного користувача. HAVING використано для перевірки >2.

![13.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/13.png)
```
-- 14. вивести список релізів з оцінкою більше середнього
...
![14.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/14.png)
```
-- 15. вивести бета-версії що знаходяться на тадії розробки
SELECT *
FROM beta_branch
...
```
Обираємо тільки бета версії які є у списку, створеному WHERE.

![15.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/15.png)
```
-- 16. вивести назви релізів, і кількість відгуків, якщо реліз має принаймні один позитивний
...
```
Аналогічно до 15. Цього разу фільтруємо за наявністю позитивних відгуків.

![16.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/16.png)
```
-- 17. вивести список проєктів з profit більше середнього
SELECT title, profit
FROM project
WHERE profit>(
	SELECT AVG(profit)
...
ORDER BY title DESC;
```
Аналогічно до 14. Порівнюємо профіт у кожному рядку із профітом розрахованим у WHERE.

![17.png](https://github.com/bedrovkyrylo/DB-intro-course/blob/main/Labs/Lab4/Pngs/17.png)
## SQL
```
